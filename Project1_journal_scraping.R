...
g3_journal = function(year){
  ...
  for(i in 1:length(indiv_url)){
    ...
    for(j in 1:length(journals_month_list)){
      ...
      #the '.' indicates the class
      Title_in = html_nodes(Title_in, xpath = '//meta[@name="DC.Title"]')
      Title_in = Title_in %>%
        #extract attribute - content
        html_attr('content')
      title = c(title,Title_in[1])
      ...
      if(length(Abstract_in)<1){
        print(article_url)
        #if not present, print "NA"
        abstract = c(abstract,"NA") 
      }else{
        #print print article url if abstract is present
        abstract = c(abstract,Abstract_in)
      }
      ...
      Corresp_Author_in = nodes9 %>% 
        #extract raw underlying text from html
        html_text() 
      corresp_author = c(corresp_author,Corresp_Author_in[1])
      ...
      Emails_in = nodes10 %>% 
        ...
        html_text() 
      #the gsub() function  used to replace the strings with input strings or values.
      email_r = gsub("{at}","@",Emails_in[1],fixed=TRUE)
      ...
      Emails_in = paste(var,collapse=",")
      emails = c(emails,Emails_in[1])
      
      #used html_nodes to extract pieces out of HTML documents
      #the '.' indicates the class
      nodes11 = html_nodes(url_new, ".aff") 
      #bring in author affiliations from html page
      Affiliates_in = nodes11 %>% 
        #extract raw underlying text from html
        html_text() 
      #collapse - an optional character string to separate the results
      affiliates = c(affiliates,paste(Affiliates_in,collapse="//"))
      
      #bring in url of the full paper from html page
      #the gsub() function  used to replace the strings with input strings or values.
      Full_Paper_in = gsub(" ","",paste(article_url,".full"))
      full_paper = c(full_paper,Full_Paper_in)
    }
  }
